...
from app.shops.avito.avito_driver import AvitoDriver
...
class AvitoDriverManager(DriverManager):
            ...
            for request in requests:
                try:
                    if self.drivers.get(request):
                        continue
                    self.drivers[request] = AvitoDriver(request, self.headless)
    ...
    def execute_drivers(self) -> Dict[str, List[AvitoAdvert]]:
        return {request: driver.get_adverts() for request, driver in self.drivers.items()}
